...
def parse_date(value: str | date | datetime) -> date:
    """Convierte una fecha de entrada a un objeto date."""
...
def cence_date(value: str | date | datetime) -> str:
    """CENCE espera fechas en formato YYYYMMDD."""
...
def normalize_demand_frame(df: pd.DataFrame) -> pd.DataFrame:
    """Ordena las columnas básicas de demanda eléctrica."""
    rename_map = {
        "fechaHora": "timestamp",
        "fecha": "timestamp",
        "MW": "mw",
...
def fetch_cence_demand(
    ...
    """Descarga datos de demanda del CENCE, día por día."""
    ...
    if not frames:
        raise ValueError("CENCE no devolvió datos de demanda para el rango solicitado")
